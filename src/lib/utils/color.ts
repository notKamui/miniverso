...
export function contrastTextForHex(hex: string): string {
  const h = hex.replace(/^#/, '')
  const r = Number.parseInt(h.length === 3 ? h[0] + h[0] : h.slice(0, 2), 16) / 255
  const g = Number.parseInt(h.length === 3 ? h[1] + h[1] : h.slice(2, 4), 16) / 255
  const b = Number.parseInt(h.length === 3 ? h[2] + h[2] : h.slice(4, 6), 16) / 255
