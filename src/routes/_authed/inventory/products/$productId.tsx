...
import { getProductQueryOptions } from '@/server/functions/inventory'

export const Route = createFileRoute('/_authed/inventory/products/$productId')({
  loader: async ({ context: { queryClient }, params: { productId } }) => {
    const data = await queryClient.fetchQuery(getProductQueryOptions(productId))
    return { crumb: data.product.name }
  },
...
function RouteComponent() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ProductForm productId={productId} onSuccess={() => navigate({ to: '/inventory' })} />
    </div>
  )
}
